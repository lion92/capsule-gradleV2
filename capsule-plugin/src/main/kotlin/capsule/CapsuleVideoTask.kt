...
"""
            ...
            val success = try {
                val result = ProcessRunner.run(
                    listOf(
                        ...
                        "-i", concatList.absolutePath, "-c", "copy", outputFile.absolutePath,
                    )
                )
                result.isSuccess && outputFile.exists()
            } catch (e: Exception) {
                false
            } finally {
                concatList.delete()
        ...
        fun createSingleSlideHtml(deckHtml: String, slideIndex: Int): String {
            val headSection = HtmlSectionParser.headMarkup(deckHtml)

            // Gourmand à dessein : la forme paresseuse s'arrête au premier
            // `</div>` imbriqué, ce qui tronque silencieusement tout deck dont
            // les diapos contiennent un `<div>` — le `</div>` du conteneur est
            // le dernier. Le compte de sections tombait alors faux et
            // l'extraction rendait le deck entier au lieu de la diapo demandée.
            // Même règle que RemotionCapture.slidesMarkup, dont c'est la source.
            val slidesContent = HtmlSectionParser.slidesMarkup(deckHtml)
                ?: return deckHtml
    ...
    internal fun captureDeckSequential(
        ...
        if (generatedVideo != null) {
            ...
            val finalOutput = convertFormatIfEnabled(finalVideo)
            logger.lifecycle("CAPSULE → {}", finalOutput.absolutePath)
        } else {
    ...
    internal fun burnInSubtitlesIfEnabled(videoFile: File, subtitleFile: File?) {
        ...
        try {
            ...
            if (burned.exists() && burned.length() > 0) {
                if (FileReplace.moveOver(burned, videoFile)) {
                    logger.lifecycle("  Subtitle burn-in: {} burned into {}", subtitleFile.name, videoFile.name)
                } else {
                    logger.error("  Subtitle burn-in: could not put '{}' in place of {} — keeping original video", burned.name, videoFile.name)
                }
            } else {
    ...
    internal fun applyAudioPostIfEnabled(finalVideo: File) {
        ...
        try {
            ...
            if (success && tmpFile.exists() && tmpFile.length() > 0) {
                if (FileReplace.moveOver(tmpFile, finalVideo)) {
                    logger.lifecycle("Audio post: BGM + loudness + ducking applied to {}", finalVideo.name)
                } else {
                    logger.error("Audio post: could not put the processed file in place of {} — keeping original video", finalVideo.name)
                }
            } else {
    ...
    internal fun injectAudio(deckFile: File, script: CapsuleScript, audioDir: File): File {
        ...
        // Le repli séquentiel réécrit le deck de zéro : construire d'abord la
        // version par `data-capsule-slide` puis la jeter était du travail perdu,
        // et relisait le fichier une seconde fois.
        val hasDataCapsuleSlide = originalHtml.contains("data-capsule-slide=")
        if (!hasDataCapsuleSlide) {
            return injectAudioSequentialFallback(originalHtml, script, audioDir, injectedDir, deckFile.name)
        }
    ...
    private fun injectAudioSequentialFallback(
        ...
        val outFile = injectedDir.resolve(deckFileName)
        outFile.writeText(injected)
        return outFile
    }
    ...
    private fun mixAudioWithVideo(videoFile: File, audioDir: File, slides: List<SlideSegment>) {
        ...
        try {
            ...
            if (result.isSuccess && tmpFile.exists()) {
                if (!FileReplace.moveOver(tmpFile, videoFile)) {
                    logger.error("  Audio mix: could not put the mixed file in place of {} — video remains silent", videoFile.name)
                    return
                }
                ...
                logger.warn("  Audio mix failed (ffmpeg exit code {}), video remains silent: {}", result.exitCode, result.tail(8))
            }
        } catch (e: Exception) {
